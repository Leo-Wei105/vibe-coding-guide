...
| ❌ 直接要求 | ✅ 思维链提示 |
|----------|-------------|
| "帮我优化这段代码" | "请一步步思考：1. 先分析功能和问题；2. 提出优化方案；3. 说明理由；4. 实现优化。在每一步都说明你的思考过程。" |

**AI会逐步展示**：第一步：分析 → 第二步：提出方案 → 第三步：说明理由 → 第四步：实现优化
...
| ❌ 没有示例 | ✅ 提供示例 |
|----------|-----------|
| "帮我写几个函数" | 提供2-3个符合你期望风格的示例 |

**示例提示词**：

```prompt
按照以下风格写函数：
示例1：function calculateSum(a, b) { return a + b; }
示例2：function greet(name) { return `Hello, ${name}!`; }
现在请按照同样风格写：1. 计算两个数的乘积 2. 判断一个数是奇数还是偶数
```
...
```prompt
你现在是一名有10年经验的代码审查员，请审查以下代码，指出所有问题并提出改进建议：[代码]
```

**示例2：架构师**

```prompt
你现在是一名系统架构师，请帮我设计一个电商系统的架构，需要考虑：高可用性、可扩展性、安全性
```
...
| 轮次 | 你的指令 | AI的响应 |
|:---:|---------|---------|
| 第一轮 | "帮我写一个登录页面" | AI生成基础版本 |
| 第二轮 | "现在添加表单验证" | AI添加验证 |
| 第三轮 | "添加错误提示，不要用alert" | AI改进提示方式 |
| 第四轮 | "添加记住密码功能" | AI添加新功能 |
...
| 类型 | 结构 | 优点 | 缺点 | 适合场景 |
|:---:|------|------|------|---------|
| **单文件应用** | `index.html` (包含所有代码) | 简单、快速 | 难以维护 | 简单项目 |
| **模块化应用** | 多目录多文件结构 | 可维护、可扩展 | 初期稍复杂 | 复杂项目 |
...
| 阶段 | 特点 |
|:---:|------|
| **初学者阶段** | 完全依赖AI写所有代码、仔细阅读AI生成的代码、理解每一行的意思 |
| **进阶阶段** | 自己写基础框架、让AI补充细节、自己理解并修改 |
| **熟练阶段** | 自己写大部分代码、让AI优化和审查、让AI帮忙解决特定问题 |
| **专家阶段** | AI作为顾问、讨论架构和方案、让AI review代码 |

### 实践2：建立代码规范

创建规范文件，让AI遵守。

创建 `.claude.md` 或 `.cursorrules` 文件：
...
|:---:|------|------|
| 步骤1 | 生成代码 | 你："帮我实现登录功能" |
| 步骤2 | 让AI审查 | 你："审查一下这段代码，指出所有潜在问题" |
| 步骤3 | 根据建议修改 | 应用AI的建议 |
| 步骤4 | 再次审查 | 你："修改后，再审查一次" |
| 步骤5 | 最终确认 | 确认代码质量后再提交 |

### 实践4：知识积累

记录有用的提示词和AI回复，创建 `prompt-library.md`：
...
在 `.claude.md` 中写入："每次创建新组件时，遵循以下结构..."
...
flowchart LR
    subgraph L1["🌱 初级"]
        A[基本操作<br/>理解代码<br/>简单项目]
    end
    
    subgraph L2["🌿 中级"]
        B[提示词技巧<br/>独立写代码<br/>复杂功能]
    end
    
    subgraph L3["🌳 高级"]
        C[提示词工程<br/>系统架构<br/>企业项目]
    end
    
    subgraph L4["⭐ 专家"]
        D[团队流程<br/>指导他人<br/>推动行业]
    end
...
| 级别 | 能力项 |
|:---:|--------|
| **初级** | ☐ 能看懂AI生成的代码 ☐ 能发现明显的错误 ☐ 能做简单修改 |
| **中级** | ☐ 能理解代码逻辑 ☐ 能独立调试问题 ☐ 能设计简单架构 |
| **高级** | ☐ 能评估代码质量 ☐ 能优化性能 ☐ 能设计复杂系统 |
